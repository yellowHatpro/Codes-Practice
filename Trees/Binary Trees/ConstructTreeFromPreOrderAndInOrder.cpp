...
TreeNode* builder(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int,int> inMap){
	...
	root->left = builder(preorder, preStart+1, preStart+range,
			inorder, inStart,inRoot-1,inMap);
	root->right = builder(preorder,preStart+1+range,preEnd,
...
TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder){
	...
	TreeNode* root = builder(preorder,0,preorder.size()-1,
