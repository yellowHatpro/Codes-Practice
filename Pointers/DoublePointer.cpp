...
int main(){
	...
	cout<<"double pointer containing address of pointer having value "<<**dptr<<endl;

	cout<<"increment functions"<<endl;
	increment1(dptr);
	cout<<**dptr<<endl; // no changes
	increment2(dptr);
	cout<<**dptr<<endl;// changes the pointer so garbage value acc to new memory location, but value changes
	increment3(dptr);
