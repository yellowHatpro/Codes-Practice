...
int editDistance_mem(string s1, string s2, int **output) {
  // helper function that uses same logic as that of the recursive solution,
  // with a bit of modification copy and paste the recursive solution
  int m = s1.size();
  int n = s2.size();
  if (s1.size() == 0 || s2.size() == 0) {
    return max(s1.size(), s2.size());
...
int editDistance_mem(string s1, string s2) {
  ...
  for (int i = 0; i <= m; i++) {
    ...
    for (int j = 0; j <= n; j++) { // Initialize matrix with -1
      output[i][j] = -1;
    }
