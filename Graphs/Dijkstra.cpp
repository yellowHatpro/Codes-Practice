...
void dijkstra(int **edges, int n) {
  ...
  for (int i = 0; i < n; i++) {
    if (distance[i] == INT_MAX) {
      cout << "Vertex " << i + 1
           << ": There is no path from the source to the destination" << endl;
    } else {
      cout << "Vertex " << i + 1 << ": shortest distance is " << distance[i]
           << endl;
    }
  }
  delete[] distance;
...
int main() {
  ...
  cin >> e;
  cout << "Note that the 1st vertex is the start vertex from which the paths "
